...
reqBucket := "catreqs"
  ...
  conn, cerr := RiakConnection(riakUrl, reqBucket)
  defer conn.Close()
  if cerr != nil {
    log.Print(cerr)
  }
 ...
 //getStruct(riakUrl, reqBucket, requestId, Request{})

   return nil
}
/*
func getStruct(rurl string, bucket string, requestId string,  element interface{}) {
  var url = []string{rurl}
  riakClient, err := db.NewRiakDB(url, bucket)
  if err != nil {
    log.Print("[x]", err)
  }
...
y := reflect.TypeOf(element)
fmt.Println(y)
  ferr := conn.FetchStruct(requestId, element)
  if ferr != nil {
     log.Print("[x]", ferr)
   }
fmt.Println("hurray")
